...
7. **Przegląd (faza 6)** — `feature-reviewer` (po wszystkich taskach `done`; → `review.md`,
   werdykt `CZYSTE`):
   ```
...
zadań przechodzą `todo → in_progress → tests_written → implemented → done`; luka decyzyjna lub
przekroczony limit iteracji → `blocked` + eskalacja (nigdy domysł).
...
2. **Przepłyń zmianę w dół** (gdy dotyka architektury/kontraktów/modelu danych — najpierw plan):
   ...
   istniejących, a taski już `done`, którym zmiana **unieważnia kryteria**, ustawia z powrotem
   na `todo` (faza 4 jest właścicielem treści `tasks.md`).
3. **Uruchom ponownie analizę (bramka 4.5)** — obowiązkowe: edycja `spec.md`/`plan.md`/`tasks.md`
   ...
   Użyj subagenta feature-analyzer dla docs/features/<slug>/.
   ```
4. **Wróć do implementacji** — dzięki idempotentności taski wciąż `done` są
...
## Ścieżka szybka i mini-bramka zakresu

`feature-quick` może pominąć `spec.md`/`plan.md`/`analysis.md` tylko dla małych zmian, które **nie**
dotykają endpointu/kontraktu API, modelu danych, reguł biznesowych ani bezpieczeństwa. Żeby to nie
było wyłącznie deklaracją agenta, minimalny `tasks.md` ścieżki szybkiej musi zawierać jawny punkt
startowy diffu oraz checklistę:

```markdown
- **Quick-scope-base**: <git-ref z początku quick path, najlepiej hash commita>
